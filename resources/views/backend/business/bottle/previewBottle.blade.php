...
<div class="container mt-2 mb-2">
  ...
  <div style="display: flex; justify-content: flex-end;">
    <a href="{{ route('dashboard.bottleMaking') }}"><button class="btn btn-sm btn-primary mt-3">Back</button></a>
  </div>
  ...
  <h3 id="description">All Bottle Making Plant</h3>
        ...
        <tbody>
            ...
            <tr>
              ...
              <td>
                ...
                <form action="{{ route('dashboard.editBottle',$data->id) }}" method="GET">
                  @csrf
                  <button type="submit" class="btn btn-sm mb-1 btn-primary">Edit</button>
                </form>

                <form action="{{ route('dashboard.removeBottle',$data->id) }}" method="POST">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="btn btn-sm mb-1 btn-danger">Delete</button>
                </form>
               
                <form action="{{ route('dashboard.activeBottle',$data->id) }}" method="POST">
                  @csrf
                  @method("PUT")
